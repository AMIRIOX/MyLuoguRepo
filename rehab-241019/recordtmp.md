* 思考方式
    ...
    12. 正难则反(`ABC-379 C`) 计数问题直接考虑答案困难, 可以求全集-补给
        例如对于 `ABC-379 C`, 可以先看作最前面有一个虚拟堆, 计算出所有棋子都在这个堆时的移动数目,
        这样其他正经位置有的棋子就相当于要移动回去抵消操作(总数目开始减)
    13. 有的题可以把问题分解, (1)可行性判断 (2)求方案数
...
* 性质/典/trick
    ...
    1. $ 1 \leq a_i \leq n $ 时考虑枚举值域, 大胆猜想
    ...
    1. 利用前缀和可以 O(1) 计算将某区间内所有数变为 x 所需的差值. (`ABC-373 E`, delta = sum - x * L)
    1. 位运算 `and/or` 操作只影响某一位, 可以分解整数的每个 bit 分别考虑
        upd: 不止 `and/or`, 任何独立的操作包括 `xor` 的运算都要考虑拆位 !!!
...
* 数据结构
    ...
    4. 求图的连通分量可以 dsu, 每个连通分量在 dsu 中的祖先作为这个连通分量的标识
        注意需要维护其他连通分量相关的数据时要扩充 dsu
